...
To use the Editor, use the [scoped package](https://docs.npmjs.com/using-npm/scope.html) to install the `@drenso-toast-ui/editor` package as you did for the previous v2.x. The following is an example of using the npm command to install the Editor. 

```sh
$ npm install @drenso-toast-ui/editor
$ npm install @drenso-toast-ui/editor@<version>
```

#### Usages

```js
const Editor = require('@drenso-toast-ui/editor'); /* CommonJS */
import Editor from '@drenso-toast-ui/editor'; /* ES6 Module */
```
...
```js
import { EditorCore } from '@drenso-toast-ui/editor'; /* ES6 Module */
...
import '@drenso-toast-ui/editor/dist/theme/toastui-editor-dark.css';
...
From v3.0, the jQuery Wrapper has been removed. To use jQuery, the user must wrap the `@drenso-toast-ui/editor` package separately.
