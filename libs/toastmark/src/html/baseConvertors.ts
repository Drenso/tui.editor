  ...
  customBlock(node, context, convertors) {
    ...
    if (customConvertor) {
      try {
        ...
        console.warn(
          `[@drenso-toast-ui/editor] - The error occurred when ${info} block node was parsed in markdown renderer: ${e}`
        );
      }
    }
  ...
  customInline(node, context, convertors) {
    ...
    if (customConvertor) {
      try {
        ...
        console.warn(
          `[@drenso-toast-ui/editor] - The error occurred when ${nomalizedInfo} inline node was parsed in markdown renderer: ${e}`
        );
      }
    }
